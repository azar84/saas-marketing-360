...
async function copyMediaSectionsNoId() {
  console.log('📷 Copying MediaSection data without ID field...');
  ...
  try {
    ...
    // Copy MediaSection data without ID field
    ...
    try {
      ...
      if (mediaSectionsCount > 0) {
        // Get media sections with correct column names (excluding ID)
        const sourceMediaSections = await sourcePrisma.$queryRaw`
          SELECT 
            position, "layoutType", "badgeText", "badgeColor", headline, subheading,
        ...
        // Insert media sections one by one (without ID)
        let successCount = 0;
        for (const mediaSection of sourceMediaSections) {
          try {
            ...
            console.log(`   ❌ Error inserting media section: ${error.message}`);
          }
        }
...
// Run the copy
copyMediaSectionsNoId().catch(console.error); 