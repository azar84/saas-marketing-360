...
// Original industries that were likely in the system
const originalIndustries = [
  ...
  // Add some common additional industries that might have been there
  {
    code: "AUTO",
    label: "Automotive",
...
async function restoreOriginalIndustries() {
  try {
    console.log('🔄 Starting to restore original industries...');
    console.log(`📊 Found ${originalIndustries.length} industries to restore`);
    
    let created = 0;
    let updated = 0;
    let skipped = 0;

    for (const industryData of originalIndustries) {
      try {
    ...
    console.log('\n✅ Industries restoration completed!');
    ...
    console.log(`\n🔍 Verification: Total active industries in database: ${totalIndustries}`);
...
if (require.main === module) {
  restoreOriginalIndustries()
    .then(() => {
      console.log('🎉 Industries restoration completed successfully!');
      process.exit(0);
    })
    .catch((error) => {
      console.error('💥 Industries restoration failed:', error);
      process.exit(1);
    });
}

module.exports = { restoreOriginalIndustries };
