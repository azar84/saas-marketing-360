...
async function copyPageBuilderDataSafe() {
  console.log('🚀 Copying page builder data safely from saski-ai-website to local database...');
  ...
  try {
    ...
    // Copy PageSection data with safe column selection
    ...
    try {
      ...
      if (pageSectionsCount > 0) {
        // Get page sections with only safe columns
...
// Run the copy
copyPageBuilderDataSafe().catch(console.error); 