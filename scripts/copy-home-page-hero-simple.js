...
async function copyHomePageHeroSimple() {
  console.log('🏠 Copying Home Page Hero data with simple approach...');
  ...
  try {
    ...
    // Copy HomePageHero data with simple approach
    ...
    try {
      ...
      if (homePageHeroCount[0].count > 0) {
        ...
        // Insert home page hero into target with simple field mapping
        let successCount = 0;
        for (const hero of sourceHomePageHero) {
          try {
            // Use only the fields that exist in both schemas
            await targetPrisma.$queryRaw`
              INSERT INTO home_page_hero (
                id, tagline, headline, subheading, "ctaPrimaryId", "ctaSecondaryId",
                "mediaUrl", "backgroundColor", "isActive", "createdAt", "updatedAt"
              ) VALUES (
                ${hero.id}, ${hero.tagline}, ${hero.headline}, ${hero.subheading},
                ${hero.ctaPrimaryId}, ${hero.ctaSecondaryId}, 
                ${hero.mediaUrl}, ${hero.backgroundValue}, ${hero.visible || true},
...
// Run the copy
copyHomePageHeroSimple().then((count) => {
  console.log('\n' + '='.repeat(80));
