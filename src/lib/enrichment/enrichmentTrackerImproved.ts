...
export class EnrichmentTrackerImproved {
  private traces: Map<string, EnrichmentTrace> = new Map();
  private activeTraces: Set<string> = new Set<string>();
  ...
  constructor() {
    ...
    console.log('📊 EnrichmentTrackerImproved initialized - writing to files in logs/enrichment/');
  }
  ...
  /**
   * Get next sequential step number for a trace
   */
  private getNextStepNumber(traceId: string): number {
    const current = this.stepCounters.get(traceId) || 0;
    const next = current + 1;
    this.stepCounters.set(traceId, next);
    return next;
  }
  ...
  startEnrichmentTrace(domain: string, normalizedDomain: string): string {
    ...
    // Reset step counter for this trace
    this.stepCounters.set(traceId, 0);
    ...
    const stepNumber = this.getNextStepNumber(traceId);
    const jobInfo = {
      traceId,
      domain,
      normalizedDomain,
    ...
    this.writeToFile(jobDir, `${stepNumber.toString().padStart(2, '0')}_job_info.json`, jobInfo);
  ...
  trackDomainValidation(traceId: string, data: {
    ...
    const jobDir = this.getJobDir(trace.normalizedDomain);
    const stepNumber = this.getNextStepNumber(traceId);
    const stepData = {
    ...
    // Write to file with sequential numbering
    this.writeToFile(jobDir, `${stepNumber.toString().padStart(2, '0')}_domain_validation.json`, stepData);
  ...
  trackPageDiscovery(traceId: string, data: PageDiscoveryData): void {
    ...
    // Write to file with sequential numbering
    this.writeToFile(jobDir, `${stepNumber.toString().padStart(2, '0')}_page_discovery.json`, stepData);
  ...
  trackPageScraping(traceId: string, data: ScrapingPageData): void {
    ...
    // Write to file with sequential numbering and URL-safe filename
    const safeUrl = data.url.replace(/[^a-zA-Z0-9.-]/g, '_').replace(/https?:\/\//, '');
    const filename = `${stepNumber.toString().padStart(2, '0')}_page_scraping_${safeUrl}.json`;
  ...
  trackWebsiteScrapingComplete(traceId: string, data: {
    ...
    // Write to file with sequential numbering
    this.writeToFile(jobDir, `${stepNumber.toString().padStart(2, '0')}_website_scraping_complete.json`, stepData);
  ...
  trackExternalAPISearch(traceId: string, data: ExternalAPISearchData): void {
    ...
    // Write to file with sequential numbering
    const filename = `${stepNumber.toString().padStart(2, '0')}_external_api_${data.source}.json`;
    this.writeToFile(jobDir, filename, stepData);
  ...
  trackGoogleEnrichment(traceId: string, data: {
    ...
    // Write to file with sequential numbering
    this.writeToFile(jobDir, `${stepNumber.toString().padStart(2, '0')}_google_enrichment.json`, stepData);
  ...
  trackLLMProcessing(traceId: string, data: LLMInteractionData): void {
    ...
    // Write to file with sequential numbering
    this.writeToFile(jobDir, `${stepNumber.toString().padStart(2, '0')}_llm_processing.json`, stepData);
  ...
  trackDataConsolidation(traceId: string, data: {
    ...
    // Write to file with sequential numbering
    this.writeToFile(jobDir, `${stepNumber.toString().padStart(2, '0')}_data_consolidation.json`, stepData);
  ...
  trackDatabaseUpsert(traceId: string, data: {
    ...
    // Write to file with sequential numbering
    this.writeToFile(jobDir, `${stepNumber.toString().padStart(2, '0')}_database_upsert.json`, stepData);
  ...
  trackMarketingTools(traceId: string, data: {
    ...
    const jobDir = this.getJobDir(trace.normalizedDomain);
    const stepDir = this.getJobDir(trace.normalizedDomain);
    const stepNumber = this.getNextStepNumber(traceId);
    const stepData = {
    ...
    // Write to file with sequential numbering
    this.writeToFile(jobDir, `${stepNumber.toString().padStart(2, '0')}_marketing_tools.json`, stepData);
  ...
  completeEnrichmentTrace(traceId: string, finalData: EnrichedCompanyData): EnrichmentTrace | null {
    ...
    // Write final summary with sequential numbering
    const jobDir = this.getJobDir(trace.normalizedDomain);
    const stepNumber = this.getNextStepNumber(traceId);
    const finalSummary = {
      traceId,
    ...
    this.writeToFile(jobDir, `${stepNumber.toString().padStart(2, '0')}_final_summary.json`, finalSummary);
    
    console.log(`✅ Enrichment trace completed: ${traceId} in ${trace.performance.totalDuration}ms`);
    console.log(`📊 Total steps tracked: ${stepNumber}`);
